...
export default async function Trampoline({ searchParams }: { searchParams: Promise<URLSearchParams> }) {
    const { deserialize } = buildParameterSerializer();
    const urlParameters = deserialize(await searchParams);

    // Add search query for Trampoline
    ...
    const { staticState, navigatorContext } = await fetchCoveoStaticState("searchEngineDefinition", {
        ...
        url: "https://sports.barca.group/trampoline",
    });
    ...
    return (
        <SearchProvider staticState={staticState} navigatorContext={navigatorContext.marshal}>
            ...
            <div className="container mx-auto px-4 py-8">
                {/* Page Header */}
                <div className="mb-8">
                    <h1 className="text-4xl font-bold text-gray-900 mb-2">Trampoline</h1>
                    <p className="text-gray-600">Find the perfect trampoline for your needs</p>
                </div>
